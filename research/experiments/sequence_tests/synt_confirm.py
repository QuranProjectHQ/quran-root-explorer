...
rng=np.random.default_rng(7); t0=time.time(); W=re.compile(r"[^\W\d_]+",re.UNICODE)
def enc(seq,cap):
...
def synt_dev(words, ndraw=3):
    base=SYNT(words); m=np.mean([SYNT(markov_once(words)) for _ in range(ndraw)]); return base-m
def windows(words,N,step,maxw=30):
    out=[]
    for c in range(0,max(1,len(words)-N+1),step):
        w=words[c:c+N]
        if len(w)<N*0.6: break
        out.append(synt_dev(w)); 
        if len(out)>=maxw: break
    return np.array(out) if out else np.array([synt_dev(words)])
...
for p in sorted(glob.glob("corpus/src_*.txt")):
    lab,w=load_g(p); nm=SCRIP.get(lab,SEC.get(lab))
...
texts["QURAN(ar)"]=qw
for N in (1200, 2500):
    print(f"\n=== window N={N} words, surrogate avg x3, seed=7 ===")
    rows=[]
    for nm,w in texts.items():
        d=windows(w,N,N//2); rows.append((d.mean(),d.std(),len(d),nm))
    for m,sd,nw,nm in sorted(rows,reverse=True):
        print(f"   {nm:16s} {m:+.3f} ± {sd:.3f}  (n={nw})")
