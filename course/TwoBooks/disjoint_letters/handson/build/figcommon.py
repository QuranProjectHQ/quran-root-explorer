...
def load():
    import os
    _cands=[os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),'Book6.xlsx'), 'Book6.xlsx', '/sessions/kind-compassionate-feynman/mnt/RootCourse/Book6.xlsx']
    _b=next((c for c in _cands if os.path.exists(c)),_cands[-1])
    wb=openpyxl.load_workbook(_b, read_only=True); ws=wb.active
